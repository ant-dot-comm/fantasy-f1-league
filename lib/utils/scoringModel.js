...
export function advancedScoring(qualifyingPos, racePos) {
    ...
    // Mover bonuses: "Big" for 11–16, "Little" for 17–22 (thresholds match 22-car field; front 6 have fewer cars to pass)
    const prefix = isFrontOfBack ? "Big " : isRearOfBack ? "Little " : "";
    if (isFrontOfBack) {
        if (positionsGained >= 10 && positionsGained <= 12) {
            driverPoints += 2;
            bonusTitle = `${prefix}Overtake Artist Bonus +2`;
        } else if (positionsGained >= 13 && positionsGained <= 14) {
            driverPoints += 3;
            bonusTitle = `${prefix}Track Titan Bonus +3`;
        } else if (positionsGained >= 15) {
            driverPoints += 4;
            bonusTitle = `${prefix}Zero to Hero Bonus +4`;
        }
    } else if (isRearOfBack) {
        if (positionsGained >= 10 && positionsGained <= 14) {
            driverPoints += 1;
            bonusTitle = `${prefix}Overtake Artist Bonus +1`;
        } else if (positionsGained >= 15 && positionsGained <= 18) {
            driverPoints += 2;
            bonusTitle = `${prefix}Track Titan Bonus +2`;
        } else if (positionsGained >= 19) {
            driverPoints += 2;
            bonusTitle = `${prefix}Zero to Hero Bonus +2`;
        }
    }
