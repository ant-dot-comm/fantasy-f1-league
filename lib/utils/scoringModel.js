...
/** Round to 2 decimal places — avoids float dust; keeps ½ / ¼ points from 1.5× and 0.75× bands. */
export function roundScorePoints(n) {
  return Math.round(n * 100) / 100;
}

/** DNF/DNS/DSQ (or position 0 / NC) — ignore classified position for scoring. */
export function isNonFinisher(result) {
...
/** Position used for scoring and display: 0 when driver did not finish. */
export function getEffectiveFinishPosition(result) {
  if (isNonFinisher(result)) return 0;
...
// Any non-finish (DNF/DNS/DSQ, including classified DNF → effective racePos 0) scores -1 point.
export function advancedScoring(qualifyingPos, racePos) {
    ...
    // Main source of points: position movement, scaled by qualifying band (fractional; not integer-rounded)
    let driverPoints;
    if (isFrontOfBack) {
        driverPoints = positionsGained * 1.5;
    } else if (isRearOfBack) {
        driverPoints = positionsGained * 0.75;
    } else {
    ...
    return {
        points: roundScorePoints(driverPoints),
        bonusTitle,
        gpWinner,
    };
}
...
export function calculateBonusPicksScore(userBonusPicks, raceResults, qualifyingResults, raceDnfs = 0) {
    ...
    if (userBonusPicks.worstDriver) {
        ...
        if (worstDriverRace) {
            ...
            const finishPosition = getEffectiveFinishPosition(worstDriverRace);
