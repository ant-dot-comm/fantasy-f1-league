...
export function advancedScoring(qualifyingPos, racePos) {
    if (racePos === 0) return { points: 0, bonusTitle: null, gpWinner: false };
    ...
    // Mover bonuses: "Big" for 11–16 (full points), "Little" for 17–22 (reduced)
    const prefix = isFrontOfBack ? "Big " : isRearOfBack ? "Little " : "";
    if (positionsGained >= 10 && positionsGained <= 13) {
        driverPoints += isFrontOfBack ? 2 : 1;
        bonusTitle = `${prefix}Overtake Artist Bonus +${isFrontOfBack ? 2 : 1}`;
    } else if (positionsGained >= 14 && positionsGained <= 17) {
        driverPoints += isFrontOfBack ? 3 : 2;
        bonusTitle = `${prefix}Track Titan Bonus +${isFrontOfBack ? 3 : 2}`;
    } else if (positionsGained >= 18) {
        driverPoints += isFrontOfBack ? 4 : 2;
        bonusTitle = `${prefix}Zero to Hero Bonus +${isFrontOfBack ? 4 : 2}`;
    }
