...
function getWorstDriverPoints(r) {
  if (!r || isNonFinisher(r)) return null;
  return -(r.startPosition - getEffectiveFinishPosition(r));
}
...
export function checkWorstDriverNailed(raceResults, bonusPicks) {
  ...
  const finishers = raceResults.filter((d) => !isNonFinisher(d));
...
export function getBigLittleCardsEarnedForUser(raceData, race, scoringModel = activeScoringModel) {
  ...
  for (const driverNumber of picks) {
    ...
    const { bonusTitle } = scoringModel(
      raceResult.startPosition,
      getEffectiveFinishPosition(raceResult)
    );
